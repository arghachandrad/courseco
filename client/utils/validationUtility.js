const validationUtility = {
  ...
  //replaceWith will be any charecter (eg: ''/,/=,-) etc
  replaceSpecialCharWith(str, replaceWith) {
    return str.replace(/[^a-zA-Z ]/g, replaceWith)
  },
  //pass total seconds and get time by hour minute second format
  hhmmConvert(value) {
  ...
  getShortName(str, char = "all") {
    if (str !== "" || str !== null || str !== undefined) {
      ...
      if (char === 2) {
        ...
        let firstLast = res[0] + "" + res[res.length - 1]
        return firstLast
      } else {
        res = str.split(" ").map((val) => {
          return val.charAt(0)
  ...
  queryParamsMapWithAPI(queryParams) {
    return queryParams
